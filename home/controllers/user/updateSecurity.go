...
// UpdateSecurity 更新用户安全密钥
func UpdateSecurity(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	params := new(updateSecurityParams)
	...
	if !templateBasic["update_security"].(bool) {
		body.ErrorJSON(w, locales.Manager.GetAdminLocales(rds, settingAdminId, acceptLanguage, "noPermissionToExecute"), -1)
		return
	}

	if utils.PasswordEncrypt(params.OldPassword) != userInfo.SecurityKey {
		body.ErrorJSON(w, locales.Manager.GetAdminLocales(rds, settingAdminId, acceptLanguage, "incorrectOldSecurityKey"), -1)
		return
	}
